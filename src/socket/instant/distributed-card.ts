                ...
                if (!getPlayer) {
                    ...
                    for (let index = 0; index < listUser.length; index++) {
                        ...
                        USERS.push({
                            ...
                            IS_DUMMY_USER: listUser[index]?.IS_DUMMY_USER,
                            AUTH_TOKEN: listUser[index]?.AUTH_TOKEN,
                        ...
                        io.of('/instant-play').to(USERS[index].CONNECTION_ID).emit('res:seven-card-distributed-instant-play', {
                            status: true,
                            sevenCard_In_InstantPlay: {
                                USER_ID: REQ_USER_ID,
                                IN_HAND_CARDS: IN_HAND_CARDS,
                                CONNECTION_ID: listUser[index]?.CONNECTION_ID,
                                IS_DUMMY_USER: listUser[index]?.IS_DUMMY_USER,
                        ...
                        if (listUser.length === index + 1) {
                            ...
                            io.of('/instant-play').in(ID).emit("res:game-start", {
                                status: true,
                                message: "Game Start.",
                                startGame_In_InstantPlay: {
                    ...
                    for (let index = 0; index < viewerUser.length; index++) {
                        USERS.push({
                            ...
                            IS_DUMMY_USER: viewerUser[index]?.IS_DUMMY_USER,
                            AUTH_TOKEN: viewerUser[index]?.AUTH_TOKEN,
                        ...
                        io.of('/instant-play').to(viewerUser[index].CONNECTION_ID).emit('res:view-seven-card-distributed-instant-play', {
                            status: true
                        })
                    }
                    ...
                    await updateAndReturnById(ID, { GAME_DECK: randomDeck, USERS: USERS, ROUND_INFO: infoRound } as RoomInstantPlay);
                }
            }
        }
    } catch (error) {
...
export { distributedCardInstantPlay };