            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    if (IS_NEW_CARD) {
                        ...
                        let updated = await updateAndReturnById(ID, { GAME_DECK: deck, DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId } as RoomInstantPlay);
                        ...
                        const DB_CURRENT_DROP_DECK = getPlayer?.CURRENT_DROP_DECK;
                        let updated = await updateAndReturnById(ID, { DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId } as RoomInstantPlay);
                        socket.emit('res:pick-card-instant-play', {
                            status: true,
                            pickCard_In_InstantPlay: {
                                PICK_CARD,
