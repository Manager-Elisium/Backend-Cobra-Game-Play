...
import { findOne, updateById as updateRoomById } from 'src/repository/room-instant-play.entity';
 ...
 */
async function devicePausedInstantPlay(io: any, socket: Socket, data: any) {
    try {
                ...
                for (let index = 0; index < getPlayer.USERS.length; index++) {
                    ...
                    if (isAuthorized.ID === USER_ID) {
                        ...
                        if (currentTurn === USER_ID) {
                            ...
                            await updateRoomById(getPlayer?.ID, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank } as RoomInstantPlay);
                        }
                        break;
                    }   
                }
...
export { devicePausedInstantPlay };
