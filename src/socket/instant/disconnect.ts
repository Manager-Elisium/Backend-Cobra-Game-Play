...
import { getRoomByConnectionId, updateAndReturnById } from 'src/repository/room-instant-play.entity';
import { deletedDisconnet } from 'src/repository/temp-instant-play.entity';
...
async function disconnectInstantPlay(io: any, socket: Socket) {
    try {
        let deleteWaitUser = await deletedDisconnet({ CONNECTION_ID: socket.id });
        if (!!deleteWaitUser?.raw?.[0]) {
            const { isAddCoin } = await addCoin({ USER_ID: deleteWaitUser?.raw?.[0]?.USER_ID, COIN: 100 });
        }
        ...
        if (!!getPlayer) {
            ...
            if (isLastPlayer?.length <= 2) {
                ...
                const getWinnerCoin = await instantGameWinner(getUserPlayRank);
                ...
                for (let index = 0; index < getPlayer?.USERS?.length; index++) {
                    ...
                    if (socket.id === CONNECTION_ID) {
                        ...
                        if (currentTurn === userId) {
                            ...
                            await updateAndReturnById(getPlayer?.ID, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank } as RoomInstantPlay);
