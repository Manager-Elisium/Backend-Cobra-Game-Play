 ...
 */
async function deviceResumedClubPlay(io: any, socket: Socket, data: any) {
    try {
        ...
        socket.to(roomId).emit('res:game-resumed-club-play', {
            status: true,
            message: 'Game resumed by another device',
            gameResumed_In_ClubPlay: {
                ROOM_ID: roomId
            }
        });
        
        console.log(`   📤 Emitted 'res:game-resumed-club-play' to room ${roomId} (excluding sender)`);
