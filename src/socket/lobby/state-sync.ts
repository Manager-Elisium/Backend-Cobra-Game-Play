...
import { findOne } from 'src/repository/room-lobby-play.entity';

/**
 * Handle state synchronization request from Unity client
 * Returns current room state for validation
 */
async function stateSyncLobbyPlay(io: any, socket: Socket, data: any) {
    try {
        // Parse request data
        let parsedData: any = {};
        try {
 ...
 */
async function fullStateLobbyPlay(io: any, socket: Socket, data: any) {
    try {
        // Parse request data
        let parsedData: any = {};
        try {
...
export { stateSyncLobbyPlay, fullStateLobbyPlay };
