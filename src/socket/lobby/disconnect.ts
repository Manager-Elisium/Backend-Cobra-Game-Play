...
import { getRoomByConnectionId, updateAndReturnById } from 'src/repository/room-lobby-play.entity';
    ...
    try {
        ...
        if (!!getPlayer) {
            ...
            if (isLastPlayer?.length <= 2) {
                ...
                const getWinnerCoin = await lobbyGameWinner(getPlayer?.ENTRY_FEES, getUserPlayRank);
