...
async function showCardLobbyPlay(io, socket, data) {
    try {
        ...
        if (!token) {
            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    console.log(`PARTICIPATED_USERS :  ${JSON.stringify(infoRound)}`);
                    if (isNextRound) {
                        let updated = await (0, room_lobby_play_entity_1.updateAndReturnById)(ID, {
                            TURN_DECIDE_DECK: [],
                            GAME_DECK: [],
                            DROP_DECK: [],
                            CURRENT_DROP_DECK: [],
                        ...
                        io.of('/lobby-play').in(ID).emit('res:show-lobby-play', {
                            status: true,
                            show_In_LobbyPlay: {
                        ...
                        const getWinnerCoin = await (0, game_winner_1.lobbyGameWinner)(getPlayer?.ENTRY_FEES, userIdList);
                        //win_game_In_LobbyPlay
                        io.of('/lobby-play').in(ID).emit('res:win-game-lobby-play', {
                            status: true,
                            win_game_In_LobbyPlay: {
                                ALL_USERS_TOTAL: showResultCard,
                                WIN_USER: minTotalObj.USER_ID,
