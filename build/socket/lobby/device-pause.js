 ...
 */
async function devicePausedLobbyPlay(io, socket, data) {
    try {
        ...
        socket.pauseTimeout = setTimeout(async () => {
            try {
                ...
                const getPlayer = await (0, room_lobby_play_entity_1.findOne)({ ID: roomName || roomId });
                if (!getPlayer) {
                ...
                for (let index = 0; index < getPlayer?.USERS?.length; index++) {
                    ...
                    if (isAuthorized?.ID === USER_ID) {
                        ...
                        if (currentTurn === USER_ID) {
                            ...
                            await (0, room_lobby_play_entity_1.updateAndReturnById)(roomName || roomId, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank });
                        }
                        break;
                    }
                }
