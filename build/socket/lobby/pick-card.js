        ...
        if (!token) {
            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    if (IS_NEW_CARD) {
                        const deck = await (0, deck_1.shuffleDeck)(getPlayer?.GAME_DECK);
                        const DrawCard = await (0, deck_1.drawCard)(deck);
                        ...
                        let updated = await (0, room_lobby_play_entity_1.updateAndReturnById)(ID, { GAME_DECK: deck, DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId });
                        socket.emit('res:pick-card-lobby-play', {
                            status: true,
                            pickCard_In_LobbyPlay: {
                                PICK_CARD: DrawCard,
                                IS_NEW_CARD
                            }
                        });
                        socket.to(ID).emit('res:animation-lobby-play', {
                            status: true,
                            animation_In_LobbyPlay: {
                        ...
                        const DB_CURRENT_DROP_DECK = getPlayer?.CURRENT_DROP_DECK;
                        let updated = await (0, room_lobby_play_entity_1.updateAndReturnById)(ID, { DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId });
                        socket.emit('res:pick-card-lobby-play', {
                            status: true,
                            pickCard_In_LobbyPlay: {
                                PICK_CARD,
...
exports.pickCardLobbyPlay = pickCardLobbyPlay;
