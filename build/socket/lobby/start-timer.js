...
async function startTimerLobbyPlay(io, socket, data) {
    try {
        console.log("***********************");
        console.log(data);
        console.log("***********************");
        const { Authtoken: token, NO_OF_PLAYER, LOBBY_ID, ENTRY_FEE, LOBBY_NAME, BUCKET_NAME, KEY } = JSON.parse(data);
        if (!token) {
            ...
            if (!isAuthorized) {
                ...
                const isDeleted = await (0, temp_lobby_play_entity_1.deleted)({ USER_ID: isAuthorized?.ID });
                // if (!Boolean(isDeleted?.affected)) {
                //     socket.emit('res:error-message', { status: false, message: "Please try again." });
                //     return;
                // }
                const { isAvalibleBalance } = await (0, reward_service_1.subtractCoin)({ USER_ID: isAuthorized.ID, COIN: ENTRY_FEE });
                if (!isAvalibleBalance) {
                    ...
                    const insertLobby = await (0, temp_lobby_play_entity_1.create)(createLobby);
                    ...
                    const countQuery = {
                        where: {
                            NO_OF_PLAYER, LOBBY_ID, ENTRY_FEE, USER_ID: (0, typeorm_1.Not)(isAuthorized?.ID)
                        }
                    };
                    let countUser = await (0, temp_lobby_play_entity_1.currentCount)(countQuery);
                    if (countUser.length === 0) {
                        ...
                        setTimeout(async () => {
                            ...
                            if (isCreateRoom.code == 1) {
                                let createRoom = await (0, room_lobby_play_entity_1.create)({
                                ...
                                if (!isCreateRoom.code) {
                                    await (0, temp_lobby_play_entity_1.deleted)({ USER_ID: isAuthorized?.ID });
 ...
 *
 */
async function playerLobbyForStart(currentPlayer) {
    try {
        const { LOBBY_ID, ENTRY_FEE, NO_OF_PLAYER, USER_ID } = currentPlayer;
        if (NO_OF_PLAYER === 2) {
            const countQuery = {
                where: {
                    NO_OF_PLAYER, LOBBY_ID, ENTRY_FEE, USER_ID: (0, typeorm_1.Not)(USER_ID)
                },
                order: {
                    CREATED_DATE: "DESC"
                },
            ...
            const countPlayerInLobby = await (0, temp_lobby_play_entity_1.currentCount)(countQuery);
            // console.log(countPlayerInLobby)
            if (countPlayerInLobby.length == 1) {
                console.log([...countPlayerInLobby, currentPlayer].map((data) => (data.USER_ID)));
                const deleteWaitRoom = await (0, temp_lobby_play_entity_1.multipleDeleted)([...countPlayerInLobby, currentPlayer].map((data) => (data.USER_ID)));
                console.log(deleteWaitRoom);
                let data = [...countPlayerInLobby, currentPlayer].map((user) => ({
                    ...user,
                    IS_JOINT_ROOM: true,
                    IS_LEAVE_ROOM: false,
                    IN_HAND_CARDS: [],
 ...
 *
 */
async function playerLobbyForLevel(currentPlayer) {
    try {
        const { LOBBY_ID, ENTRY_FEE, NO_OF_PLAYER, USER_ID } = currentPlayer;
        ...
        let getPlayer = await (0, temp_lobby_play_entity_1.findOneLobby)(findCurrentPlayerQuery);
        ...
        if (!!getPlayer) {
            if (NO_OF_PLAYER === 2 && getPlayer.IS_LOBBY == true && getPlayer.NO_OF_PLAYER === NO_OF_PLAYER) {
                ...
                if (countPlayerInLobby.length == 1) { // Create Room
                    const deleteWaitRoom = await (0, temp_lobby_play_entity_1.multipleDeleted)([...countPlayerInLobby, currentPlayer].map((data) => (data.USER_ID)));
                    // console.log(deleteWaitRoom)
                    let data = [...countPlayerInLobby, getPlayer].map((user) => ({
                        ...user,
                        IS_JOINT_ROOM: true,
                        IN_HAND_CARDS: [],
                        TOTAL: 0,
