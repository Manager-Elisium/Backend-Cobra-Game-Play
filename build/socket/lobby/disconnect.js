...
async function disconnectLobbyPlay(io, socket) {
    try {
        let deleteWaitUser = await (0, temp_lobby_play_entity_1.deletedDisconnet)({ CONNECTION_ID: socket.id });
        if (!!deleteWaitUser?.raw?.[0]) {
            const { isAddCoin } = await (0, reward_service_1.addCoin)({ USER_ID: deleteWaitUser?.raw?.[0]?.USER_ID, COIN: deleteWaitUser?.raw?.[0]?.ENTRY_FEE });
        }
        let getPlayer = await (0, room_lobby_play_entity_1.getRoomByConnectionId)({ CONNECTION_ID: socket.id });
        if (!!getPlayer) {
            ...
            if (isLastPlayer?.length <= 2) {
                ...
                let updated = await (0, room_lobby_play_entity_1.updateAndReturnById)(getPlayer?.ID, {
                    TURN_DECIDE_DECK: [],
                    GAME_DECK: [],
                    DROP_DECK: [],
                    CURRENT_DROP_DECK: [],
                ...
                const getWinnerCoin = await (0, game_winner_1.lobbyGameWinner)(getPlayer?.ENTRY_FEES, getUserPlayRank);
                io.of('/lobby-play').to(minTotalObj?.CONNECTION_ID).emit('res:win-game-lobby-play', {
                    status: true,
                ...
                for (let index = 0; index < getPlayer?.USERS?.length; index++) {
                    ...
                    if (socket.id === CONNECTION_ID) {
                        ...
                        if (currentTurn === userId) {
                            ...
                            io.of('/lobby-play').in(getPlayer?.ID).emit('res:next-turn-lobby-play', {
                                status: true,
                                nextTurn_In_LobbyPlay: {
                                    CURRENT_TURN: nextUserId
                                }
                            });
                            await (0, room_lobby_play_entity_1.updateAndReturnById)(getPlayer?.ID, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank });
                        }
