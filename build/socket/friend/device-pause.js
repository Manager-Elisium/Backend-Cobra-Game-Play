 ...
 */
async function devicePausedFriendPlay(io, socket, data) {
    try {
        ...
        socket.pauseTimeout = setTimeout(async () => {
            try {
                ...
                const getPlayer = await (0, room_friend_play_entity_1.findOne)({ NAME: roomName || roomId });
                if (!getPlayer) {
                ...
                isLastPlayer = await getPlayer?.USERS?.filter((data) => (!data.IS_LEAVE_ROOM && data.TOTAL < 100));
                ...
                if (isLastPlayer.length <= 2) {
                    ...
                    const winnerId = isLastPlayer?.find((data) => data?.USER_ID !== isAuthorized.ID)?.USER_ID;
                    ...
                    const updated = await (0, room_friend_play_entity_1.updateAndReturnById)(getPlayer?.ID, {
                        TURN_DECIDE_DECK: [],
                        GAME_DECK: [],
                        DROP_DECK: [],
                        CURRENT_DROP_DECK: [],
                    ...
                    if (currentTurn === isAuthorized.ID) {
                        const currentIndex = isLastPlayer?.findIndex((user) => user.USER_ID === isAuthorized.ID);
                        ...
                        await (0, room_friend_play_entity_1.updateById)(getPlayer?.ID, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank });
                    }
                    else {
                        await (0, room_friend_play_entity_1.updateById)(getPlayer?.ID, { USERS: getPlayer.USERS, USER_WIN_RANK: getUserPlayRank });
                    }
