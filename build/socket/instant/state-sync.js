...
exports.fullStateInstantPlay = exports.stateSyncInstantPlay = void 0;
 ...
 */
async function stateSyncInstantPlay(io, socket, data) {
    try {
        // Parse request data
        let parsedData = {};
        try {
        ...
        const room = await (0, room_instant_play_entity_1.findOne)({ NAME: roomName });
        if (!room) {
        ...
        if (room.USERS && Array.isArray(room.USERS)) {
            activePlayers = room.USERS.filter((user) => user && !user.IS_LEAVE_ROOM);
...
exports.stateSyncInstantPlay = stateSyncInstantPlay;
/**
 * Handle full state update request from Unity client
 * Returns complete room state for recovery
 */
async function fullStateInstantPlay(io, socket, data) {
    try {
        // Parse request data
        let parsedData = {};
        try {
...
exports.fullStateInstantPlay = fullStateInstantPlay;
