...
// Leave Player is not Win
// Leave 
async function leaveRoomInstantPlay(io, socket, data) {
    try {
        console.log(`Request Player :::: `, JSON.parse(data));
        ...
        if (!token) {
            ...
            if (!isAuthorized) {
                ...
                console.log(`IS LAST Player :::: `, isAuthorized?.ID, ID);
                if (!getPlayer) {
                    ...
                    if (isLastPlayer.length <= 2) {
                        console.log(`IS LAST Player :::: `, isLastPlayer.length);
                        ...
                        for (let index = 0; index < getPlayer.USERS.length; index++) {
                            ...
                            if (isAuthorized.ID === USER_ID) {
                                ...
                                if (currentTurn === USER_ID) {
                                    ...
                                    await (0, room_instant_play_entity_1.updateAndReturnById)(ID, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank });
                                }
                                break;
                            }
                        }
                        // TODO : Is Leave Room
                        io.of('/instant-play').in(ID).emit('res:leave-room-instant-play', {
                            status: true,
