            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    if (IS_NEW_CARD) {
                        ...
                        let updated = await (0, room_instant_play_entity_1.updateAndReturnById)(ID, { GAME_DECK: deck, DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId });
                        socket.emit('res:pick-card-instant-play', {
                            status: true,
                            pickCard_In_InstantPlay: {
                                PICK_CARD: DrawCard,
                                IS_NEW_CARD
                            }
                        });
                        socket.to(ID).emit('res:animation-instanst-play', {
                            status: true,
                            animation_In_InstancePlay: {
                        ...
                        const DB_CURRENT_DROP_DECK = getPlayer?.CURRENT_DROP_DECK;
                        let updated = await (0, room_instant_play_entity_1.updateAndReturnById)(ID, { DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId });
                        socket.emit('res:pick-card-instant-play', {
                            status: true,
                            pickCard_In_InstantPlay: {
                                PICK_CARD,
                    ...
                    io.of('/instant-play').in(ID).emit('res:next-turn-instant-play', {
                        status: true,
