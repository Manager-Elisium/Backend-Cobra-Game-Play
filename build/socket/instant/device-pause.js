...
exports.devicePausedInstantPlay = void 0;
 ...
 */
async function devicePausedInstantPlay(io, socket, data) {
    try {
        ...
        const room = io.of('/instant-play').adapter.rooms.get(roomId);
        ...
        socket.pauseTimeout = setTimeout(async () => {
            try {
                ...
                const getPlayer = await (0, room_instant_play_entity_1.findOne)({ ID: roomName || roomId });
                if (!getPlayer) {
                    ...
                    if (isAuthorized.ID === USER_ID) {
                        ...
                        if (currentTurn === USER_ID) {
                            ...
                            io.of('/instant-play').in(getPlayer?.ID).emit('res:next-turn-instant-play', {
                                status: true,
                                nextTurn_In_InstantPlay: {
                                    CURRENT_TURN: nextUserId
                                }
                            });
                            await (0, room_instant_play_entity_1.updateById)(getPlayer?.ID, { USERS: getPlayer.USERS, CURRENT_TURN: nextUserId, USER_WIN_RANK: getUserPlayRank });
                        }
                        break;
                    }
                }
                // Notify all clients in room
                io.of('/instant-play').in(getPlayer?.ID).emit('res:leave-room-instant-play', {
                    status: true,
        ...
        socket.to(roomId).emit('res:game-paused-instant-play', {
            status: true,
            message: 'Game paused by another device',
            gamePaused_In_InstantPlay: {
                ROOM_ID: roomId
            }
        });
        console.log(`   📤 Emitted 'res:game-paused-instant-play' to room ${roomId} (excluding sender)`);
...
exports.devicePausedInstantPlay = devicePausedInstantPlay;
