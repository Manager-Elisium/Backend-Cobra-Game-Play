...
async function leaveTimerInstantPlay(io, socket, data) {
    try {
        ...
        if (!token) {
            ...
            if (!isAuthorized) {
                ...
                let deleteWaitUser = await (0, temp_instant_play_entity_1.deleted)({ USER_ID: isAuthorized.ID });
                const { isAddCoin } = await (0, reward_service_1.addCoin)({ USER_ID: isAuthorized.ID, COIN: 100 });
