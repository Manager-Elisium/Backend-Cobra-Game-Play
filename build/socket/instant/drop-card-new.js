            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    let remainingCards = user.IN_HAND_CARDS.filter((card) => {
                        let matchingCards = DROP_CARD.filter((removeCard) => removeCard.rank.name === card.rank.name &&
                    ...
                    const newArray = getPlayer?.USERS?.map((user) => user?.USER_ID === isAuthorized.ID
                        ...
                        : user);
                    const isShow = getPlayer?.USERS?.filter((data) => data.IS_LEAVE_ROOM || data.TOTAL >= 100)?.map((user) => user.IN_HAND_CARDS.length == 1);
                    if (isShow.every((data) => data == true)) {
                        ...
                        let cardDistributedLength = filterUser.findIndex((data) => data.USER_ID == isAuthorized?.ID);
