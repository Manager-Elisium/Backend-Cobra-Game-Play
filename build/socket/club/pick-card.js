...
async function pickCardTablePlay(io, socket, data) {
    try {
            ...
            if (!isAuthorized) {
                ...
                if (!getPlayer) {
                    ...
                    if (IS_NEW_CARD) {
                        ...
                        let updated = await (0, room_club_play_entity_1.updateAndReturnById)(ID, { GAME_DECK: deck, DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId });
                        ...
                        const DB_CURRENT_DROP_DECK = getPlayer?.CURRENT_DROP_DECK;
                        let updated = await (0, room_club_play_entity_1.updateAndReturnById)(ID, { DROP_DECK: DROP_DECK, CURRENT_DROP_DECK: [], PREVIOUS_DROP_DECK: getPlayer?.CURRENT_DROP_DECK, USERS: newArray, CURRENT_TURN: nextUserId });
                        socket.emit('res:pick-card-table-play', {
                            status: true,
                            pickCard_In_TablePlay: {
                                PICK_CARD,
